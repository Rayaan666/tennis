...
export default function Footer({ onOpenBooking }) {
  ...
  return (
    <footer className="relative w-full bg-[#050505] text-[#F5F5F0] border-t border-white/10 pt-20 pb-12 overflow-hidden">
      
      {/* Minimal Court-Line Graphic Running Across Footer */}
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#276B0B]/40 to-transparent" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="pb-16 border-b border-white/10 flex flex-col lg:flex-row lg:items-end justify-between gap-8">
          <div className="flex items-center gap-6">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-2 border-[#276B0B]/40 p-1 bg-black shrink-0 overflow-hidden shadow-xl">
              <img
                src="/assets/logo.png"
            ...
            <div>
              <span className="font-mono text-xs text-[#276B0B] tracking-court uppercase block mb-1">
              ...
              <h2 className="font-display font-black text-4xl sm:text-6xl md:text-7xl uppercase text-white tracking-tighter leading-none">
                LION ELITE <br />
                <span className="text-[#276B0B]">TENNIS ACADEMY</span>
              </h2>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <button
              ...
              className="px-8 py-4 bg-[#276B0B] hover:bg-white text-[#050505] font-display text-sm font-bold tracking-wider uppercase transition-colors"
            >
        ...
        <div className="py-16 grid grid-cols-2 md:grid-cols-4 gap-10 lg:gap-12">
          ...
          <div>
            <div className="flex items-center gap-2 mb-6">
              <span className="w-2 h-2 bg-[#276B0B]" />
            ...
            <ul className="space-y-3 font-display text-base tracking-wide uppercase">
              {quickLinks.map((link) => (
                <li key={link.name}>
                  <a
                    href={link.href}
                    className="text-[#B9B9B9] hover:text-[#276B0B] transition-colors flex items-center gap-2 group"
                  >
                    <span>{link.name}</span>
                    <span className="w-0 h-[1px] bg-[#276B0B] transition-all group-hover:w-3" />
                  </a>
                </li>
              ))}
            </ul>
              ...
              {programLinks.map((prog) => (
                <li key={prog.id}>
                  <button
                    ...
                    className="text-[#B9B9B9] hover:text-[#276B0B] transition-colors text-left"
                  >
                    {prog.name}
                  </button>
                </li>
          ...
          <div>
              ...
              {socialLinks.map((social) => (
                <li key={social.name}>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[#B9B9B9] hover:text-[#276B0B] transition-colors flex items-center justify-between"
                  >
                    <span>{social.name}</span>
                    <ArrowUpRight className="w-3.5 h-3.5 text-neutral-500 group-hover:text-[#276B0B]" />
                  </a>
                </li>
              ))}
            </ul>
            ...
            <div className="mt-8 p-3 bg-[#111] border border-white/10 font-mono text-[10px] text-neutral-400">
              <span className="text-[#276B0B] block mb-1">OPERATING HOURS</span>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4 font-mono text-[11px] text-neutral-500">
          ...
          <div className="flex items-center gap-4 text-[10px]">
            ...
            <span>•</span>
            <span className="text-[#276B0B]">BUILT FOR THE NEXT POINT.</span>
          </div>
        </div>
